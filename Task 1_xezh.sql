...
insert into Members(MEMBER_ID, NAME, EMAIL, PHONE_NO, ADDRESS, MEMBERSHIP_DATE) values(1,"Alice","10100@example.com","123456","123 street","2024-01-15"),(2,"Bob","12345@example.com","342342","Oak Avenue","2024-02-20"),(3,"Brown","Brown@example.com","232434","999 road","2024-03-10"),(4,"Lily","Lily@example.com","203802","YY street","2014-03-25");
insert into BorrowingRecords(BORROW_ID,MEMBER_ID,BOOK_ID,BORROW_DATE,RETURN_DATE) values(1,1,2,"2025-07-01",NULL),(2,1,2,"2025-06-15","2015-07-15"),(3,2,3,"2025-06-01",NULL),(4,3,4,"2025-05-10",NULL),(5,3,5,"2025-07-20",NULL),(6,4,2,"2025-05-01","2025-05-20"),(7,1,6,"2025-08-01",NULL),(8,3,3,"2019-09-20","2020-01-02");
select * from Books;
select* from Members;
select* from BorrowingRecords;
# Information Retrieval
# a) Retrieve a list of books currently borrowed by a specific member.
select Books.TITLE from BorrowingRecords BR left join Books on BR.BOOK_ID=Books.BOOK_ID
...
select GENRE, count(AVAILABLE_COPIES)AS Borrow_Availabe from Books group by GENRE;
...
select B.AUTHOR, count(*) as borrow_numbers
from Books B left join BorrowingRecords BR on B.BOOK_ID=BR.BOOK_ID
group by B.AUTHOR
having count(*)>=10
order by borrow_numbers DESC;
